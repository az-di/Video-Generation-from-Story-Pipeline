from video_meta_prompt import video_meta_prompt
...
input_folder = os.path.join(base_path, f"chapter_{chapter}_images")
input_scene_file = os.path.join(base_path, f"scenes_ch_{chapter}.json")
video_prompt_file = os.path.join(base_path, f"video_prompts_ch_{chapter}.json")
output_folder = os.path.join(base_path, f"generated_videos_ch_{chapter}")
os.makedirs(output_folder, exist_ok=True)


clients = [
    os.getenv("VIDEO_ENDPOINT_1"),
    os.getenv("VIDEO_ENDPOINT_2"),
    os.getenv("VIDEO_ENDPOINT_3"),
    os.getenv("VIDEO_ENDPOINT_4"),
    os.getenv("VIDEO_ENDPOINT_5"),
    os.getenv("VIDEO_ENDPOINT_6")
]


# Setup clients and locks
client_objs = [Client(url) for url in clients]
...
def generate_video_prompt(image_path, scene_title, core_content, bible_lines, core_action):
    prompt_filled = video_meta_prompt.replace("{{ $('Loop Over Items').item.json.scene_title }}", scene_title)
    prompt_filled = prompt_filled.replace("{{ $('Set Image').item.json.corecontent }}", core_content)
    prompt_filled = prompt_filled.replace("{{ $('Set Image').item.json.bible_lines }}", bible_lines)
    prompt_filled = prompt_filled.replace("{{ $('Set Image').item.json.core_action }}", core_action)
    ...
    response = client.chat.completions.create(
        ...
        messages=[
            {
                "role": "system",
                "content": "You are a helpful chatbot assistant that converts biblical chapters and images into image prompts."
            },
            {
                "role": "user",
                "content": [
    ...
    for image_path in result_files:
        ...
        while trycnt < maxretry:
            ...
            try:
                ...
                core_content = scene.get("core_content", "")
                bible_lines = scene.get("bible_lines", "")
                core_action = scene.get("core_action", "")
                vid_length = scene.get("scene_length", 5) 

                print(index + "    " + image_path)
                
                generated_prompt_json = generate_video_prompt(
                    image_path, scene_title, core_content, bible_lines, core_action
                )
...
if __name__ == "__main__":
   ...
   # store_video_prompts() # generate and store all video prompts
    ...
    with ThreadPoolExecutor(max_workers=len(clients)) as executor:  # 4 clients = 4 threads
        future_to_info = {}
